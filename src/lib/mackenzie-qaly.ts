 ...
 * MacKenzie Scott's lifetime philanthropy. The Python package is the source of
 * truth; the parameters here are imported verbatim from its exported
 * `params.json` (generated by `msqaly-export-params`). The math mirrors
 * `model.py` / `distributions.py` exactly; only the RNG differs (a seeded
 * mulberry32 here vs numpy's PCG64), so results match in distribution — not
 * bit-for-bit — which is all a Monte Carlo estimate claims anyway.
 *
