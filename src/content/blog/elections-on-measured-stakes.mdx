...
the welfare metric ranks first — the status quo — never appears. Two
options also collapse the voting-rules question: score the policy you
prefer 5 and the other 0, and you've cast a plurality ballot. So the
model puts current law on the ballot as a third option and runs every
rule it implements — plurality, approval under both threshold
conventions, 0–5 score under two normalizations, STAR, instant
runoff — across the same noise grid.

![Probability each voting rule enacts the welfare-optimal option, against perception noise](./elections-on-measured-stakes-rules.png)

At perfect information the knife-edge vanishes for every rule. The 76%
whose entire stake was the $4.10 levy now have somewhere to put it:
current law, which charges them nothing. Plurality tracks perfectly at
σ = 0. Ballot design — an agenda offering the near-indifferent mass
only two levies it disliked — produced the two-option failure, not
anything about how plurality counts.

One rule still fails, and the failure is an instruction, not a formula.
Strict approval — approve what looks *better than* the status quo — can
never approve the status quo itself, so the credit program's
beneficiaries elect it unopposed: zero tracking, $169.29 of
equally-distributed-equivalent income lost per household, the worst
cell in the table. Change one word — approve anything *at least as
good* — and tracking goes from 0% to 100%. When three in four voters
are near-indifferent, the ballot instruction decides the
perfect-information outcome.

Add noise and the ranking reverses. From σ = $3,000 up, strict approval
leads — 0.883 tracked, $19.75 mean regret against plurality's $60.66 —
because the threshold that broke it at σ = 0 now filters out ballots
that are pure noise. Plurality degrades worst (0.642 at $3,000, 0.367
at $30,000). And scoring in proportion to dollar stakes, the
referendum-like version, buys no reliable advantage over spending the
full 0–5 scale.

No rule dominates: plurality is exact at perfect information and worst
under noise, strict approval the reverse. And every gap between rules
is smaller than what the agenda decides — which options reach the
ballot beats how the ballot counts them. Full table:
[docs/rules.md](https://github.com/MaxGhenis/democrasim/blob/strategic/docs/rules.md).

## A dial instead of a ballot

Discrete platforms ask whether elections rank policies correctly. A
continuous policy asks something different: how much do voters demand,
and why. The dial: position t between 0 and 1 raises every federal
bracket rate by t × 10 points and returns the revenue as an equal
per-adult transfer. At the top of the dial the engine scores $1,286.6B
of revenue — a $4,809 check per adult per year — 68.6% of households
come out ahead, and the Gini falls from 0.4541 to 0.4135.

![Inequality and transfers along the redistribution dial](./elections-on-measured-stakes-axis.png)

Voters here care about outcomes, not dial positions: each holds an
ideal Gini, a dollar price on missing it, and a selfish weight on their
own household. What they know enters as a slope — a voter with belief λ
thinks the dial moves the Gini λ times as far as it does. Because the
measured Gini falls almost linearly in t, that one number divides
demand. Want half the achievable inequality reduction and believe
policy half as effective as it is? You demand the entire axis — the
maximalist's platform, held for the opposite reason.

![Demanded dial position against belief in the policy's reach](./elections-on-measured-stakes-demand.png)

The same voter demands 0.650 at λ = 0.75, exactly 0.500 at λ = 1, 0.325
at λ = 1.5, 0.250 at λ = 2. "What policy do you support?" separates
values from beliefs only if you measure both.

Elections don't average the errors away. Give the whole electorate one
goal — a target Gini of 0.4338, half the dial's reach — and split it
between correct voters (λ = 1) and skeptics who think policy does a
quarter of what it does (λ = 0.25). With up to 40% skeptics, two
office-seeking candidates converge on exactly 0.500 — what a fully
correct electorate enacts — even though at 40% the average belief runs
30% too weak. At 50% the equilibrium jumps to 0.750; from 60% it pins
to the maximum. Everywhere in the sweep, the equilibrium enacts the
electorate's median ideal — pure Downs — which is exactly why the
median belief, not the mean, sets the outcome. On the two-policy
ballot, independent misperception canceled; on a continuum the median
belief rules. Same model, same data, opposite aggregation.

And self-interest never moderates. A purely self-interested electorate
enacts t = 1 at every own-stake noise level tested ($0, $1,000,
$10,000), because 68.6% of households gain: mean net stakes run from
+$7,777 in the bottom income decile to −$35,671 in the top, crossing in
the eighth. That's Meltzer and Richard's classic demand for
redistribution, running on measured incidence. Moderation on this dial
has to come from values — an ideal short of maximum equality — or from
beliefs. And the engine holds behavior fixed, so more redistribution
always raises the welfare metric and the inequality-averse planner's
optimum sits at the corner: this layer models demand formation, not
optimal taxation. Full numbers:
[docs/axis.md](https://github.com/MaxGhenis/democrasim/blob/strategic/docs/axis.md).
...
offer as a ballot option (the position game does, and chooses it).
Ballots stay sincere under every voting rule, and the dial holds
behavior fixed — no labor response — so the planner's optimum sits at
the corner and every interior result on it comes from ideal points, not
efficiency costs. The
...
[voting rules](https://github.com/MaxGhenis/democrasim/blob/strategic/docs/rules.md),
[the dial](https://github.com/MaxGhenis/democrasim/blob/strategic/docs/axis.md) —
