...
description: 'An election model where every input except perception is measured. At perfect information, the winner turns on a $4-a-year financing residual.'
pubDate: 'Jul 11 2026'
...
welfare ranking is a modeling choice — log utility with a $1,000 income
floor ranks A first, and inequality aversion of η ≥ 3 flips it — and
both financed policies score below the status quo, which plurality
doesn't offer as a ballot option. The
[repo](https://github.com/MaxGhenis/democrasim) carries the
[full findings note](https://github.com/MaxGhenis/democrasim/blob/main/docs/findings.md),
...
The missing measurement is perception. The model says what happens *if*
voters see their stakes with σ = $500 or a $200 shared bias; it can't
say where real voters sit. That's a survey — elicit what people believe
specific reforms would do to their own household, compute what the tax
engine says, and fit the gap
([democrasim#3](https://github.com/MaxGhenis/democrasim/issues/3)). Its
answer will be one number on the axis you just dragged.
